    ...
    private static class PixelGraph implements Graph<Node> {
        ...
        public class Pixel implements Node {
            ...
            public List<Edge<Node>> neighbors(Picture picture, EnergyFunction f) {
                ...
                {
                    Pixel rmid = new Pixel(x + 1, y);
                    result.add(new Edge<>(this, rmid, f.apply(picture, x + 1, y)));
                    if (y != 0) {
                        Pixel rup = new Pixel(x + 1, y - 1);
                        result.add(new Edge<>(this, rup, f.apply(picture, x + 1, y - 1)));
                    }
                    if (y != picture.height() - 1) {
                        Pixel rdown = new Pixel(x + 1, y + 1);
                        result.add(new Edge<>(this, rdown, f.apply(picture, x + 1, y + 1)));
                    }
                }
